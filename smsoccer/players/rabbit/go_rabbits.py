...
from smsoccer.players.rabbit.rabbitmaster import RabbitMaster
from smsoccer.players.rabbit.rabbitwall import RabbitWall
from smsoccer.players.rabbit.rabbitspeeder import RabbitSpeeder
...
def spawn_agent(team_name, goalie):
    ...
    """
    try:
        a = RabbitWall() if goalie else RabbitSpeeder()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print "args: ./go_rabbits.py <team_name> <num_players>"
        sys.exit()
    ...
    for agent in xrange(min(10, int(sys.argv[2]) - 1)):
        print "  Spawning Rabbit Speeder %d..." % agent
